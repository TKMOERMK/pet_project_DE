...
def get_and_transfer_raw_data_to_ods_pg(**context):
    ...
    if SOURCE == "weather":
        con.sql(
            f"""
            INSERT INTO dwh_postgres_db.{SCHEMA}.fct_weather (time, temperature_2m, rain)
            SELECT
                time,
                temperature_2m,
                rain
            FROM 's3://prod/{LAYER}/{SOURCE}/{start_date}/{start_date}_*.parquet';
            """
        )
        ...
        logging.info("Inserted earthquake data into %s.%s for %s", SCHEMA, TARGET_TABLE, start_date)
